...
const ProductCard = ({ product, viewMode = 'grid' }) => {
  ...
  const handleAddToCart = (e) => {
    e.preventDefault(); // Stop navigation to product details
    addToCart(product);
  };
      ...
      <div className="group w-full flex border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
        ...
        <div className="relative w-1/3 min-w-[120px] md:min-w-[200px] bg-gray-50 flex-shrink-0">
             {discountPercentage > 0 && (
                <span className="absolute top-2 left-2 bg-[#734F96] text-white text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm z-10">
                  {discountPercentage}%
                </span>
             )}
        ...
        <div className="p-4 flex flex-col flex-grow">
           ...
           <Link to={`/product/${product.id}`}>
              <h3 className="text-base font-semibold text-gray-800 hover:text-[#734F96] mb-2 line-clamp-2">{product.name}</h3>
           </Link>
           ...
           <div className="mt-auto flex items-center justify-between">
              <div className="flex flex-col">
                 ...
                 <span className="text-lg font-bold text-[#734F96]">₹{price.toLocaleString()}</span>
              </div>
              <button 
                  className="bg-[#734F96] text-white p-2 rounded-lg hover:bg-[#5e3f7a] transition-colors shadow-sm flex items-center gap-2 text-sm font-medium px-4"
  ...
  return (
    <div className="group w-full h-full flex flex-col rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-300">
      <div className="relative p-3 md:p-4 flex-shrink-0">
        {discountPercentage > 0 && (
          <span className="absolute top-3 left-3 bg-[#734F96] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md z-10 shadow-sm">
            {discountPercentage}%
          </span>
        )}
      ...
      <div className="px-3 md:px-4 pb-3 md:pb-4 flex flex-col flex-grow space-y-1.5">
        ...
        <Link to={`/product/${product.id}`} className="block">
            <h3 className="text-xs sm:text-sm font-semibold text-gray-800 line-clamp-2 leading-snug hover:text-[#734F96] transition-colors h-8 md:h-10 overflow-hidden">
            {product.name}
            </h3>
        </Link>
        ...
        <div className="flex items-center gap-2 mt-auto pt-1">
          ...
          <span className="text-[#734F96] font-bold text-sm md:text-lg">₹{price.toLocaleString()}</span>
        </div>

        <button 
            className="mt-2 w-full flex items-center justify-center gap-1.5 rounded-lg border border-[#734F96] text-[#734F96] font-bold text-[10px] md:text-[13px] py-2 hover:bg-[#7D2596] hover:text-white transition-all duration-200 active:scale-95"
            onClick={handleAddToCart}
        >
