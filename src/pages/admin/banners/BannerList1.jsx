...
import { Trash2, Edit, Plus } from 'lucide-react'; // Using standard icons
...
export default function BannerList1() {
  ...
  // Fetch only type 'home_1'
  const fetchBanners = async () => {
    const data = await getBanners('home_1');
    setBanners(data);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 bg-[#f8f9fa] min-h-screen font-sans">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/admin/banners/home-1/add" className="bg-[#3b82f6] text-white px-6 py-2.5 rounded font-bold text-sm hover:bg-blue-700 shadow-md uppercase">
          Add Banner
        </Link>
      </div>
              ...
              <tr key={banner.id} className="hover:bg-gray-50">
                <td className="p-4">
                  <div className="bg-gray-100 rounded-lg p-2 inline-block border border-gray-200">
                    <img src={banner.imageUrl} alt="" className="h-32 object-contain" />
                  </div>
                </td>
