...
const Shop = () => {
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen font-sans pb-24 md:pb-0"> 
        ...
        `}>
          <div className="h-full p-5 md:p-0 bg-white md:bg-transparent overflow-y-auto">
             ...
             <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                <h3 className="font-bold text-gray-800 mb-4 text-sm border-b pb-2">Shop by Category</h3>
                <ul className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                  <li className="flex items-center gap-3 cursor-pointer" onClick={() => handleCategoryClick('All')}>
                     <div className={`w-4 h-4 border rounded flex items-center justify-center ${activeCategory === 'All' ? 'bg-[#734F96] border-[#734F96]' : 'border-gray-300'}`}>
                        {activeCategory === 'All' && <Check size={12} className="text-white" />}
                     </div>
                     <span className={`text-sm ${activeCategory === 'All' ? 'text-[#734F96] font-bold' : 'text-gray-600'}`}>All Products</span>
                  </li>
                  {categories.map((cat) => (
                    <li key={cat.id} className="flex items-center gap-3 cursor-pointer" onClick={() => handleCategoryClick(cat.id)}>
                       <div className={`w-4 h-4 border rounded flex items-center justify-center ${activeCategory === cat.id ? 'bg-[#734F96] border-[#734F96]' : 'border-gray-300'}`}>
                          {activeCategory === cat.id && <Check size={12} className="text-white" />}
                       </div>
                       <span className={`text-sm ${activeCategory === cat.id ? 'text-[#734F96] font-bold' : 'text-gray-600'}`}>{cat.name}</span>
                    </li>
                  ))}
                </ul>
             </div>
             ...
             <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                <h3 className="font-bold text-gray-800 mb-4 text-sm border-b pb-2">Filter By Price</h3>
                <div className="px-1 py-2">
                   <input 
                      type="range" min="0" max="30000" step="100" 
                      value={priceRange} 
                      onChange={(e) => setPriceRange(Number(e.target.value))}
                      className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#734F96]"
                   />
                   <div className="flex justify-between mt-3 text-sm text-gray-600 font-bold">
                      <span>₹0</span>
                      <span className="text-[#734F96]">₹{priceRange.toLocaleString()}</span>
                   </div>
                </div>
             </div>
             ...
             {/* Rating Filter */}
             <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                <h3 className="font-bold text-gray-800 mb-4 text-sm border-b pb-2">Filter By Rating</h3>
                <ul className="space-y-2">
                   {[5, 4, 3, 2, 1].map((star) => (
                      <li key={star} className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-1 rounded" onClick={() => setSelectedRating(star === selectedRating ? 0 : star)}>
                         <div className={`w-4 h-4 border rounded flex items-center justify-center ${selectedRating === star ? 'bg-[#734F96] border-[#734F96]' : 'border-gray-300'}`}>
        ...
        <main className="flex-1 w-full">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 sticky top-16 md:static z-20">
             <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
                <div className="flex items-center gap-2 border-r pr-4 border-gray-300">
                   <button onClick={() => setViewMode('list')} className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-gray-100' : 'text-gray-400'}`}><List size={20} /></button>
                   <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-gray-100' : 'text-gray-400'}`}><Grid size={20} /></button>
                </div>
                <span className="text-gray-500 text-sm">Showing <b>{filteredProducts.length}</b> products</span>
             </div>

             <div className="relative w-full md:w-auto">
                <button onClick={() => setShowSortDropdown(!showSortDropdown)} className="flex items-center gap-2 border px-3 py-1.5 rounded text-sm bg-white font-medium w-full md:min-w-[160px] justify-between hover:border-[#734F96] transition-colors">
                ...
                {showSortDropdown && (
                    <div className="absolute right-0 top-full mt-1 w-full md:w-48 bg-white border rounded shadow-lg z-50">
                       {['Name, A to Z', 'Name, Z to A', 'Price, Low to High', 'Price, High to Low'].map(opt => (
                           <div key={opt} onClick={() => { setSortBy(opt); setShowSortDropdown(false); }} className={`p-2 text-sm cursor-pointer hover:bg-purple-50 ${sortBy === opt ? 'text-[#734F96] font-bold' : ''}`}>{opt}</div>
                       ))}
                    </div>
                )}
             </div>
          ...
          {loading ? (
             ...
             <div className="text-center py-20 bg-white border rounded-lg">
                 ...
                 <button onClick={() => {setPriceRange(30000); setSearchParams({})}} className="text-[#734F96] font-bold mt-2 hover:underline">Clear Filters</button>
             </div>
          ) : (
      ...
      <button 
        className="md:hidden fixed bottom-20 right-4 z-50 bg-[#734F96] text-white w-12 h-12 rounded-full shadow-2xl flex items-center justify-center hover:scale-105 transition-transform"
