...
import { 
  Star, ShoppingCart, Heart, Share2, Plus, Minus, ChevronRight, 
  CheckCircle, Truck, ShieldCheck, Loader, User 
} from 'lucide-react';
...
export default function ClientProductDetails() {
  ...
  // ✅ 2. PREPARE SCHEMA DATA (Safe fallback if product is null)
  const schemaProduct = product || {
    name: "Loading Product...",
    description: "Please wait...",
    price: "0",
    imageUrl: "https://ignitenow.tech/logo.png" // Replace with your default logo URL
  };

  const cleanDescription = schemaProduct.description 
    ? schemaProduct.description.replace(/<[^>]+>/g, '').substring(0, 160) + '...'
    : 'Product Details';
  ...
  const productSchema = {
    ...
    "image": mainImage || schemaProduct.imageUrl,
    "description": cleanDescription,
    "brand": {
      "@type": "Brand",
      "name": schemaProduct.brand || "Ignite Insights"
    },
    "sku": id,
    "offers": {
      "@type": "Offer",
      ...
      "availability": (Number(schemaProduct.stock) || 0) > 0 
  ...
  // ✅ 3. RENDER HELMET *BEFORE* THE LOADING CHECK
  return (
    <>
      <SEO 
        title={product ? `${product.name} - Buy Online` : 'Loading...'} 
        description={cleanDescription}
        image={mainImage}
        url={`/shop/${id}`}
        type="product"
      ...
      {/* ✅ 4. LOADING CHECK FOR UI ONLY */}
      {loading ? (
        <div className="min-h-screen flex items-center justify-center text-[#7D2596]"><Loader className="animate-spin" /></div>
      ) : !product ? (
